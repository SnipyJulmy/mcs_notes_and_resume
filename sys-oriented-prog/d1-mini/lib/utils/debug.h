...
// -- Logging
    ...
    #define log_info(M, ...)         fprintf(stdout, BLUE_COLOR "[INF]: " M "\n", ##__VA_ARGS__)
    #define log_info_full(M, ...)    fprintf(stdout, BLUE_COLOR "[INF] (%s:%d): " M "\n", __FILE__, __LINE__, ##__VA_ARGS__)
    #define log_info_mul(io_statements) do {\
                                fprintf(stdout, BLUE_COLOR "[INF] ");\
                                io_statements\
                                fprintf(stdout, COLOR_X);\
                                } while(0);
    #define log_info_print(M, ...)    fprintf(stdout, BLUE_COLOR M COLOR_X, ##__VA_ARGS__)
    ...
    #define debug2(M, ...)      fprintf(stdout, GREEN "\t[DBG]: " M "\n" COLOR_X, ##__VA_ARGS__)
